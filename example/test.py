...
if __name__ == '__main__':
    ...
    image = io.imread("tt.png",as_grey = True)
    image = np.where(image > 0.99, 1,0)
    ...
    print((2,5),'in up or top base lines:', judge_point_in_base_segmentation(2,15,uper_segment,down_segment))


    # plt.show()    pp = plt.subplot(111).imshow(image)

    tem, only_point = [], []
    # upget_next_feature_point(image.astype(np.uint8), 12, 13, 3, 14, tem, only_point)
    image = image.astype(np.uint8)
    upget_next_feature_point(image,5,8,5,9,tem,only_point)
    ...
    point = totalpoint[0]
    point = np.array(point)
    p5.plot(point[:, 1], point[:, 0], "b.-")
    ########藏文历史文献中黏连字丁串的切分与识别########
    p6 = plt.subplot(236)
    point = totalpoint[0]
    point = np.array(point)
    p6.plot(point[:, 1], point[:, 0], "g.-")
