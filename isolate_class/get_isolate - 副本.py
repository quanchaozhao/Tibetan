        ...
        for j in range(i + 1,(len(totallist)),1):
            ...
            if (confidence_value > 0.4):
                if(totallist[i][0][1] < totallist[j][0][1] and totallist[i][0][3] > totallist[j][0][3]) or (totallist[j][0][1] < totallist[i][0][1] and totallist[j][0][3] >totallist[i][0][3]):
                    ...
                    totallist[i][0] = [min_y1,min_x1,max_y2,max_x2]
                    totallist[j][2] = 2
        io.imsave("D:\\pycharm\\C-Process-photo\\datebase\\test\\" + str(i) + str(j) + ".png",tem.astype(np.int8) * 255, cmap="gray")
        totallist[i][2] = 1
...
if __name__ == "__main__":
    ...
    for i in os.listdir(path):
        ...
        if file.endswith('.png'):
            ...
            # p1 = plt.subplot(212)
            print(file,"DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD")
            isolate_character(image,file)
