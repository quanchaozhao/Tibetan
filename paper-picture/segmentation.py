...
def show_point(file):
    ...
    for item in corners:
        ...
        if ((x <= int(col / 3 + 0.5) or x >= int(col / 3 * 2 + 0.5 - 2)) and col <= 80):
            print(x, 'now x id')
            continue
        if ((x <= int(col / 4 + 0.5) or x >= int(col / 4 * 3 + 0.5)) and col > 80):
            continue
        point_flag = True
        if (tt[y][x] == 0):
    ...
    p1.plot([pointx[0], pointx[2]], [pointy[0], pointy[2]], 'r^', linewidth=8.0)
    p1.plot([pointx[1]], [pointy[1]], 'rv', linewidth=8.0)
    plt.axis('off')
    plt.show()
    total_image, alter_x, alter_y = isolate_character(img / 255, result_pointy, result_pointx, False)
    ...
    plt.axis('off')
    tk.plot([xxx[1], xxx[2], xxx[5]], [yyy[1], yyy[2], yyy[5]], 'rv', linewidth=4.0)
    tk.plot([xxx[0], xxx[3], xxx[4]], [yyy[0], yyy[3], yyy[4]], 'r^', linewidth=4.0)
...
result_txt = r'result_outline_418_three.txt'
