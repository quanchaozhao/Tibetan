...
def falter_image(path):
    for i in os.listdir(path):
        ...
        if(row > 100 or col > 100 ):
            os.remove(file)
            print(file ,"OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
            continue
...
if __name__ == "__main__":
    train_path = "D:\\datebase\\isolate_div\\5\\"
    result_path = "D:\\datebase\\cluster\\5\\"
    PCA_Kmeans(train_path,result_path)
    # # Uses sparse pca codepath.
    # # imlist = imtools.get_imlist('C:/Users/Zqc/Desktop/PCV-book-data/data/selectedfontimages/a_selected_thumbs/')
    #
    # # falter_image("D:/datebase/test")
    #
    # imlist = imtools.get_imlist('D:/datebase/test14/')
    # print(imlist)
    # # 获取图像列表和他们的尺寸
    # im = array(Image.open(imlist[0]))  # open one image to get the size
    # m, n = im.shape[:2]  # get the size of the images
