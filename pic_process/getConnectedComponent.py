...
def alterXml(fileName, info, path='d:'):
    ...
    pg = ET.SubElement(root, 'Page',
                       attrib={'imageHeight': imageHeight, 'imageWidth': imageWidth, 'imageFilename': fileName,
                               'class': ',', 'strokeWidth': '8'})
    for i in range(len(pointx)):
        textRegion = ET.SubElement(pg, 'TextRegion')
        coods = ET.SubElement(textRegion, 'Coords')
        for j in range(len(pointx[i])):
            for k in range(len(pointx[i][j])):
              ET.SubElement(coods, 'Point', attrib={'x': str(pointx[i][j][k]), 'y': str(pointy[i][j][k])})
    tree = ET.ElementTree(root)
    fileName = fileName.replace('_anonymous@unknown.com', '')
        ...
        for i in range(len(pointx)):
            ...
            for j in range(0, len(pointx[i]) - 1, 2):
                ...
                if (len(x) > 0 and len(y) > 0):
                    ...
                    tem_x.append(x)
                    tem_y.append(y)
            alter_x.append(tem_x)
            alter_y.append(tem_y)
            image[mark_x, mark_y] = 0
