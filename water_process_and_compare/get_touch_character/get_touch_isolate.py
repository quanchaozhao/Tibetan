...
path_Xml = r"C:\Users\Zqc\Desktop\第二次数据-寒假\First_date_total_Alter"
path_image = r"C:\Users\Zqc\Desktop\第二次数据-寒假\First_date_total_Alter"
Alter_Xml_path = r"C:\Users\Zqc\Desktop\第二次数据-寒假\test"
def alter_point(point):
    for i in range(len(point)):
        for j in range(len(point[i])):
            point[i][j] = int((point[i][j] - 15) / 3)
    return point
def get_part_image(XmlFile,isDisplayPart = False,isAlterXml = False):
    ...
    if(not isAlterXml):
        ...
        alterXml(fileName,[imageHeight,imageWidth,(alter_y),(alter_x)],Alter_Xml_path)
...
if __name__ == "__main__":
    ...
    for i in os.listdir(path_Xml):
        ...
        if file.endswith('.xml'):
            print(file)
            get_part_image(file,isAlterXml=True)