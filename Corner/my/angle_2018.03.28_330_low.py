...
def find_segmention_point(image_or, index, pointx, pointy, position_point):
    ...
    for i in range(len(reslut) - 1):
        ...
        for j in range(i + 1, len(reslut), 1):
            if (reslut[j][3] == 0 and reslut[i][3] == 0 and reslut[i][2] != reslut[j][2]):
                if (abs(reslut[i][0] - reslut[j][0]) * abs(reslut[i][0] - reslut[j][0]) +
                    ...
                    max = abs(reslut[i][0] - reslut[j][0]) * abs(reslut[i][0] - reslut[j][0]) / 4 + abs(
                        reslut[i][1] - reslut[j][1]) * abs(reslut[i][1] - reslut[j][1])
                    tem = j
        ...
        point_flag = True
        t = reslut[i][0]
        print('DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD', str(result[1][2]))
        if (reslut[i][2] == 'up'):
            t = t + 1
            while (below_image[t][reslut[-1][1]] == 0):
                t = t + 1
            for k in range(t + 1, row, 1):
                if (below_image[k][reslut[i][1]] == 0):
                    print("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUU")
                    if not (check_near_point(t, reslut[i][1], k - 1, reslut[i][1])):
                        break
                    result_segmentation_point.append([t, reslut[i][1], k - 1, reslut[i][1]])
                    reslut[i][3] = 1
                    flag = False
                    break
        elif (reslut[i][2] == 'down'):
            t = t - 1
            while (below_image[t][reslut[i][1]] == 0):
                t = t - 1
    ...
    if (reslut[-1][2] == 'up' and reslut[-1][3] == 0):
        ...
        for k in range(t + 1, row, 1):
            if (below_image[k][reslut[-1][1]] == 0):
                ...
                if not (check_near_point(t, reslut[-1][1], k - 1, reslut[-1][1])):
                    break
                result_segmentation_point.append([t, reslut[-1][1], k - 1, reslut[-1][1]])
                reslut[-1][3] = 1
                break
        ...
        for k in range(t - 1, -1, -1):
            if (below_image[k][reslut[-1][1]] == 0):
                ...
                reslut[-1][3] = 1
                break
    elif (reslut[-1][2] == 'neardown' and reslut[-1][3] == 0):
        for k in range(t - 1, -1, -1):
            if (below_image[k][reslut[-1][1]] == 0):
                print("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD2")
                if not (check_near_point(t, reslut[-1][1], k + 1, reslut[-1][1])):
                ...
                reslut[-1][3] = 1
                break
    elif ((reslut[-1][2] == 'nearup' or result[-1][2] == 'up') and reslut[-1][3] == 0):
        for k in range(t + 1, row, 1):
            if (below_image[k][reslut[-1][1]] == 0):
                print("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUU2")
                if not (check_near_point(t, reslut[-1][1], k - 1, reslut[-1][1])):
