...
def del_near_point(pointx,pointy):
    ...
    for i in range(len(pointx) - 1):
        ...
        for j in range(i + 1,len(pointx),1):
            ...
            if(pointx[i] - pointx[j]) * (pointx[i] - pointx[j]) + (pointy[i] - pointy[j]) * (pointy[i] - pointy[j]) < 9:
                flag = False
                break;
        if(flag):
...
def show_point(file):
    ...
    tt = (get_outline(fill_image((get_fileter_image(adjunction_image(np.where(img / 255 > 0.9, 1, 0)))))))
    ...
    for item in corners:
        ...
        for i in range(len(end_pointx)):
            if ((end_pointy[i] - x) * (end_pointy[i] - x) + (end_pointx[i] - y) * (end_pointx[i] - y) < 25) and (abs(y - index) >= 3):
                point_flag = False
                break;
            elif(((end_pointy[i] - x) * (end_pointy[i] - x) + (end_pointx[i] - y) * (end_pointx[i] - y) > 25) and (index - y >= 2)):
                vewol = vewol + 1
...
def find_segmention_point(image_or, index, pointx, pointy, position_point):
    ...
    #             image[j][i] = 0
    #             image_or[j][i] = 0
    below_image = image_or.astype(np.uint8)
    result_segmentation_point = []
    # reslut = [[x, y, position, 0] for (x, y, position) in (pointx, pointy, position_point)]
    reslut = []
    for i in range(len(pointx)):
        reslut.append([pointx[i], pointy[i], position_point[i], 0])
    print("result:        ",reslut)
    for i in range(len(reslut) - 1):
        ...
        if (reslut[i][3] == 1):
            continue
        for j in range(i + 1,len(reslut),1):
            if(reslut[j][3] == 0 and reslut[i][3] == 0 and reslut[i][2] != reslut[j][2]):
                if(abs(reslut[i][0] - reslut[j][0]) * abs(reslut[i][0] - reslut[j][0]) +
                        abs(reslut[i][1] - reslut[j][1]) * abs(reslut[i][1] - reslut[j][1]) < max):
                    max = abs(reslut[i][0] - reslut[j][0]) * abs(reslut[i][0] - reslut[j][0]) / 4 + abs(reslut[i][1] - reslut[j][1]) * abs(reslut[i][1] - reslut[j][1])
                    tem = j

        if (max < 64):
            result_segmentation_point.append([reslut[i][0], reslut[i][1], reslut[tem][0], reslut[tem][1]])
            reslut[i][3] = 1
            reslut[tem][3] = 1
            continue
        if(reslut[i][2] == 'up'):
            for k in range(reslut[i][0] + 1,row, 1):
                below_image[k][reslut[i][1]] == 0
                if(below_image[k][reslut[i][1]] == 0):
                    print("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUU")
                    if not(check_near_point(reslut[i][0], reslut[i][1], k - 1, reslut[i][1])):
                        break
                    result_segmentation_point.append([reslut[i][0], reslut[i][1], k - 1, reslut[i][1]])
                    reslut[i][3] = 1
                    flag = False
                    break
        else:
            for k in range(reslut[i][0] - 1,-1,-1):
                if(below_image[k][reslut[i][1]] == 0):
    ...
    if (reslut[-1][2] == 'up' and reslut[-1][3] == 0):
        for k in range(reslut[-1][0] + 1, row, 1):
            if (below_image[k][reslut[-1][1]] == 0):
                print("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUU")
                if not (check_near_point(reslut[i][0], reslut[i][1], k - 1, reslut[i][1])):
                    break
                result_segmentation_point.append([reslut[-1][0], reslut[-1][1], k - 1, reslut[-1][1]])
                reslut[-1][3] = 1
                break
    elif(reslut[-1][2] == 'down' and reslut[-1][3] == 0):
        for k in range(reslut[-1][0] - 1, -1, -1):
            if (below_image[k][reslut[-1][1]] == 0):
                print("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD")
                if not (check_near_point(reslut[i][0], reslut[i][1], k + 1, reslut[i][1])):
                    break
