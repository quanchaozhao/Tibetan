...
def find_segmention_point(image_or, index, pointx, pointy, position_point):
    ...
    for i in range(len(result) - 1):
        ...
        for j in range(i + 1, len(result), 1):
            if (result[j][3] == 0 and result[i][3] == 0 and result[i][2] != result[j][2]):
                if (abs(result[i][0] - result[j][0]) * abs(result[i][0] - result[j][0]) +
                    ...
                    max = abs(result[i][0] - result[j][0]) * abs(result[i][0] - result[j][0]) / 2.25 + abs(
                        result[i][1] - result[j][1]) * abs(result[i][1] - result[j][1]) * 1.5
                    tem = j
        ...
        if (result[i][2] == 'up'):
            # t = t + 1
            # while (t < row and below_image[t][result[i][1]] == 0):
            #     t = t + 1
            # if (t == row):
            #     t = result[i][0]
...
def filter_near_point(pointx,pointy):
    pointx_tem = []
    pointy_tem = []
    for i in range(len(pointx) - 1):
        flag = True
        for j in range(len(pointx_tem)):
            if( abs(pointx[i] - abs(pointx_tem[j])) < 8):
                print('del near points')
                flag = False
                break
        if(flag):
            pointx_tem.append(pointx[i])
            pointy_tem.append(pointy[i])
    return pointx_tem,pointy_tem
def show_point(file):
    global totalpoint
    ...
    if (col <= 80):
        ...
        corners = cv2.goodFeaturesToTrack((tt).astype(np.uint8), 30, 0.001, 0.05, blockSize=4)
    ...
    for item in corners:
        ...
        # if ((x <= int(col / 3 + 0.5) or x >= int(col / 3 * 2 + 0.5 - 2)) and col <= 80):
        #     print(x, 'now x id')
        #     continue
        # if ((x <= int(col / 5 + 0.5) or x >= int(col / 5 * 4 + 0.5)) and col > 80):
        #     continue
        ...
        for i in range(len(end_pointx)):
            if (((end_pointy[i] - x) * (end_pointy[i] - x) + (end_pointx[i] - y) * (end_pointx[i] - y)) < 50) and (
                    abs(y - index) >= 2):
                point_flag = False
                count_num = count_num + 1
                print(x,y,'end point',[end_pointy[i],end_pointx[i]])
                break;
            elif (((end_pointy[i] - x) * (end_pointy[i] - x) + (end_pointx[i] - y) * (end_pointx[i] - y) > 50) and (
                    index - y >= 2)):
                vewol = vewol + 1
        if (not point_flag):
            continue
        ...
        pointx.append(x)
        pointy.append(y)
    print(count_num,'ffffffffnumnumun')
    ...
    pointx = pointx + alter_pointx
    pointy = pointy + alter_pointy
    pointx,pointy = filter_near_point(pointx,pointy)
    ...
    if (len(pointx) == 0):
        ...
        for item in corners:
            ...
            if (abs(y - index) < 3 and not (total_point(tt, y, x) > 3)):
                continue
            if (x <= int(col / 3 + 0.5 + 1) or x >= int(col / 3 * 2 + 0.5 - 1) and col < 80):
                continue
            if (x <= int(col / 4 + 0.5 + 2) or x >= int(col / 4 * 3 + 0.5 - 2) and col >= 80):
                continue
            if (3 < abs(y - index) < 8):
                pointx.append(x)
                pointy.append(y)
    ...
    for i in range(len(result_point)):
        p2.plot([result_point[i][1], result_point[i][3]], [result_point[i][0], result_point[i][2]], 'b.-')
...
result_txt = r'result_outline_423_two.txt'
wrfile = open(result_txt, 'r')
totalpoint = []
label = ["g.-", "b.-", "r.-", "k.-", "w.-", "c.-"]
...
for i in os.listdir(path):
    if i.endswith('.png'):
        ...
        show_point(file)
wrfile.close()
print('TT cost:', time.strftime('%M-%S', time.localtime(time.time() - nowtime)))
# tem, only_point = [], []
