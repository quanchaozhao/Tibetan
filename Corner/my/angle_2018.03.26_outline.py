...
def judge_vowel(image,index):
    ...
    if(index > 10):
        ...
        for region in regions:
            ...
            ratio,classification = get_class_and_ratio(region.image.astype(np.uint8) * 255)
...
def show_point(file):
    ...
    result_point = find_segmention_point(img_filter, index, pointy, pointx, position_point)
    # print(pointx)
    # print(pointy)
...
def find_segmention_point(image_or, index, pointx, pointy, position_point):
    ...
    for i in range(len(reslut) - 1):
        ...
        for j in range(i + 1,len(reslut),1):
            if(reslut[i][3] == 1):
                break
            if(reslut[j][3] == 0 and reslut[i][3] == 0 and
                    abs(reslut[i][0] - reslut[j][0]) < 15 and abs(reslut[i][1] - reslut[j][1]) < 10):
                result_segmentation_point.append([reslut[i][0], reslut[i][1],reslut[j][0], reslut[j][1]])
                reslut[i][3] = 1
                reslut[j][3] = 1
                break
            if(reslut[i][2] == 'up'):
